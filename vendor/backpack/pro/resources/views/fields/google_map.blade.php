...
@push('crud_fields_styles')
    @bassetBlock('backpack/pro/fields/google-map-field.css')
    <style>
...
@push('crud_fields_scripts')
    ...
    <script>
        function bpFieldInitGoogleMapElement(element) {
            ...
            const savePosition = (pos, address, mapField) => {
                new Promise(resolve => {
                    var data = {};
                    data['lat'] = typeof pos.lat === 'function' ? pos.lat() : pos.lat;
            ...
            try {
                ...
                marker.addListener('dragend', function (e) {
                    getAddressAndSavePosition(marker.position, mapField, searchInput);
                });
                ...
                if(hasLocation) {
                    ...
                    searchBox.addListener("places_changed", () => {
                        ...
                        marker.position = places[0].geometry.location;
                    });
                }
    ...
    @loadOnce('google_places_api_script')
        <script async defer src="https://maps.googleapis.com/maps/api/js?v=3&key={{ $field['api_key'] ?? config('services.google_places.key') }}&loading=async&libraries=places,marker&callback=initGoogleAddressAutocomplete&language={{$field['map_options']['language']}}"></script>
    @endLoadOnce
@endpush
