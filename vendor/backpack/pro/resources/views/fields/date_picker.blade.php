...
<?php
    ...
    $field['date_picker_options']['language'] = $field['date_picker_options']['language'] ?? \App::getLocale();
?>

@include('crud::fields.inc.wrapper_start')
    <input type="hidden" class="form-control" bp-field-main-input name="{{ $field['name'] }}" value="{{ $field['value'] }}">
...
@push('crud_fields_scripts')
    ...
    @if ($field['date_picker_options']['language'] !== 'en')
        @basset('https://cdn.jsdelivr.net/npm/bootstrap-datepicker@1.9.0/dist/locales/bootstrap-datepicker.'.$field['date_picker_options']['language'].'.min.js')
    @endif

    @bassetBlock('backpack/pro/fields/date-picker-field.js')
    <script>
        if (jQuery.ui) {
        ...
        function bpFieldInitDatePickerElement(element) {
            ...
            $picker.on('keydown', function(e){
                // only allow arrows and enter and esc keys as per docs: https://bootstrap-datepicker.readthedocs.io/en/latest/keyboard.html#up-down-left-right-arrow-keys
                if($.inArray(e.keyCode, [37, 38, 39, 40, 13, 27]) === -1){
                    $fake.blur();
                    $picker.bootstrapDP('hide')
                }   
            });
